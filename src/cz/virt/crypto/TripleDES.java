...
public class TripleDES {
	...
	private static String algorithmCipher = "DESede/CBC/NoPadding";	
	private final static char[] hexArray = "0123456789ABCDEF".toCharArray();

	private static SecretKey getKey(String keyValue) throws Exception {
	...
	private static byte[] getKey16bitArray(String keyVal) {
		...
		for (int i = 0; i < len; i += 2) {
			result[i / 2] = (byte) ((Character.digit(keyVal.charAt(i), 16) << 4)
					+ Character.digit(keyVal.charAt(i + 1), 16));
		}
		byte[] key;
		if (result.length == 16) {
			key = new byte[24];
	...
	public static String getVDSP(String data) {
		return fillString( new String(" ") + new String(" ") + getRandomChar(4) + data.length() + data, 8, '!'); //VISA format
	}
	
	public static String getFromVDSP(String data) {
		String trimSalt = data.substring(6, data.length());		
